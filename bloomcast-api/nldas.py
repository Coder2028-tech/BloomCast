...
class EarthdataSession(requests.Session):
    ...
    def __init__(
        ...
        super().__init__()
        self.headers.update({"User-Agent": "BloomCastNJ/0.1"})
        self.trust_env = True
    ...
    def rebuild_auth(self, prepared_request, response) -> None:
        ...
        if (
            original_url.hostname != redirect_url.hostname
            and redirect_url.hostname != self.AUTH_HOST
            and original_url.hostname != self.AUTH_HOST
        ):
...
def earthdata_session(
    ...
    return EarthdataSession(username=username, password=password)
    ...
    with session.get(url, stream=True, allow_redirects=True, timeout=120) as response:
        try:
            ...
            if response.status_code == 401:
                raise RuntimeError(
                    "NASA Earthdata rejected the request. Check EARTHDATA_USERNAME/EARTHDATA_PASSWORD, "
                    "reload .env, and make sure your Earthdata account has authorized GES DISC access."
                ) from exc
            if response.status_code == 404:
                raise RuntimeError(
                    "NASA GES DISC could not find the requested NLDAS file. Check the date range and data availability."
                ) from exc
            raise
...
def open_nldas_dataset(
    ...
    kwargs = {"combine": "by_coords", "combine_attrs": "override", "chunks": chunks}
    if engine:
